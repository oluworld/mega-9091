...
/**
 * Finds the last .snapshot file by number and finds all the subsequent pending
 * .log files.
 */
class NumberFileFinder {
    ...
    /**
     * @throws IOException if location does not exist and cannot be created as a
     * directory.
     * @throws IllegalArgumentException If location exists and is not a
     * directory.
     */
    public NumberFileFinder(String directoryName) throws IOException {
        this.directory = new File(directoryName);
        if (!directory.exists() && !directory.mkdirs()) {
            throw new IOException("Directory doesn't exist and could not be created: " + directoryName);
        }
        if (!directory.isDirectory()) {
            throw new IllegalArgumentException("Path exists but is not a directory: " + directoryName);
        }
    ...
    /**
     * Returns a NumberFileCreator that will start off with the first number
     * after the last .log file number.
     *
    ...
    private long number(File snapshot) throws NumberFormatException {  //NumberFomatException is a RuntimeException.
        ...
        if (!name.endsWith("." + NumberFileCreator.SNAPSHOT_SUFFIX)) {
