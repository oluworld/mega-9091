...
/**
 * Provides transparent persistence for business objects. This applies to any
 * deterministic system implementing the PrevalentSystem interface. All commands
 * to the system must be represented as objects implementing the Command
 * interface and must be executed using Prevayler.executeCommand(Command). Take
 * a look at the demo application included with the Prevayler distribution for
 * examples.
 *
...
public class SnapshotPrevayler implements Prevayler {
    ...
    /**
     * Returns a new Prevayler for the given PrevalentSystem. "PrevalenceBase"
     * shall be the directory where the snapshot and log files shall be created
     * and read.
     *
     * @param newSystem The newly started, "empty" PrevalentSystem that will be
     * used as a starting point for every system startup, until the first
     * snapshot is taken.
     */
     ...
     * used as a starting point for every system startup, until the first
     * snapshot is taken.
     * @param directory The full path of the directory where the snapshot and
     * log files shall be created and read.
     */
     ...
     *
     * @see system()
     * @return The serializable object that was returned by the execution of
     * command.
