 ...
 *  Copyright (C) 1998-2004 Radim Kolar
 *
...
public final class mgr implements Runnable {
    ...
    /* 
 * default save timer is 3 min, you can set it higher 
 * experiments shows that 5 mins is too much and that going
 * under 3 mins is too risky.
     */
    public static final long SAVETIMER = 1000L * 60 * 3;
    ...
    public static String ftp_proxy_auth;
    /* auth header */
    ...
    public static gnu.rex.Rex regex_pass[];
    ...
    mgr() {
        dircache = new Hashtable<String, cachedir>(40);
        ...
        /* default squid parent proxy port */
        ui.uiport = 0;
        ui.uiadr = null;
        ...
        ui.minsearchcachettl = 30 * 1000L;
        ourport = 8080;
        /* well known webcache port     */
        ...
        min_age = 30 * 60 * 1000L;
        /* 30 minut */
        max_age = 7 * 24 * 60 * 60 * 1000L;
        /* 7 dnu */
        expire_age = 1000L * 60 * 5;
        /* 5 minutes */
        redir_age = 15 * 1000L;
        /* 15 seconds */
        request.qa_minlen = -1;
        /* quick abort disable */
        request.qa_maxtime = 1 * 60 * 60 * 1000L;
        /* max luxovaci cas 1 min */
        httpreq.client_timeout = 30 * 1000;
        /* 30 sec. read from cl. timeout */
        request.read_timeout = 4 * 60 * 1000;
        /* 4 min. read from net timeout  */
        request.request_timeout = 45 * 1000;
        /* 45 seconds, like Opera */
        cacheobject.keep_deleted = false;
        cacheobject.overwrite_by_redirect = true;
        ...
        clear_flags_on_shutdown = true;
        trace_fail = false;
        trace_remap = false;
        trace_redirect = false;
        httpreq.trace_timestamp = false;
    ...
    public void go() {
        ...
        saver.start();
        /* start background directory saver... */
        ...
        if (scache.faststart == false) {
            ...
            if (lv == null) {
                lv = "(Error connecting to server)";
            } else if (!lv.equals(scache.CACHEVER)) {
                lv += "\007 ... its time for update!";
            } else {
                lv += " ... up to date";
            }
        ...
        if (clear_flags_on_start) {
            checkFlag(shutdownflag, true); // delete pending shutdown flag on start
            checkFlag(immediate_shutdownflag, true); // delete pending shutdown flag on start
        }
    ...
    private boolean endDotFilesystem() {
        ...
        for (String name : names) {
            if (name.toLowerCase().equals("dot.")) {
                return true;
            }
        }
    ...
    public final void read_config(String cfgfile) {
        try {
            ...
            while ((line = dis.readLine()) != null) {
                ...
                try {
                    ...
                    if (token.equalsIgnoreCase("visible_link")) {
                        httpreq.visible_link = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("stat_log")) {
                        stat_log = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("visible_hostname")) {
                        httpreq.visible_hostname = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("ui_hostname")) {
                        ui.ui_hostname = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("default_forward_for")) {
                        request.default_forward_for = st.nextToken();
                        continue;
                    }

                    if (token.equalsIgnoreCase("defaultname")) {
                        cacheobject.defaultname = st.nextToken();
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("bindaddress")) {
                        ...
                        try {
                            ...
                            System.err.println("[SMARTCACHE-ERROR] Can not resolve my BindAddress: " + ip);
                        }
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("ui_bindaddress")) {
                        ...
                        try {
                            ...
                            System.err.println("[SMARTCACHE-ERROR] Can not resolve my UI BindAddress: " + ip);
                        }
                        continue;
                    }

                    if (token.equalsIgnoreCase("shutdown_flag")) {
                        shutdownflag = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("alive_flag")) {
                        aliveflag = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("mime_types")) {
                        repair.loadMimeTypes(st.nextToken());
                        continue;
                    }

                    if (token.equalsIgnoreCase("immediate_shutdown_flag")) {
                        immediate_shutdownflag = st.nextToken();
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("http_port")) {
                        ourport = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }

                    if (token.equalsIgnoreCase("ui_port")) {
                        ui.uiport = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }
                    if (token.equalsIgnoreCase("loader_add_missing")) {
                        ui.loader_add_missing = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }
                    if (token.equalsIgnoreCase("loader_add_refresh")) {
                        ui.loader_add_reloads = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }
                    if (token.equalsIgnoreCase("loader_queue_file")) {
                        ui.loader_queue_file = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("loader_refresh_depth")) {
                        ui.loader_refresh_depth = st.nextToken();
                        continue;
                    }
                    if (token.equalsIgnoreCase("loader_add_depth")) {
                        ui.loader_add_depth = st.nextToken();
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("fake_user_agent")) {
                        request.fake_user_agent = "User-Agent: " + st.nextToken("\r\n").trim();
                        continue;
                    }

                    if (token.equalsIgnoreCase("fake_referer")) {
                        request.fake_referer = "Referer:" + st.nextToken("\r\n").trim();
                        request.referer_hack = 98;
                        continue;
                    }

                    if (token.equalsIgnoreCase("swap_level1_dirs")) {
                        swap_level1_dirs = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }

                    if (token.equalsIgnoreCase("swap_level2_dirs")) {
                        swap_level2_dirs = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("nocompress")) {
                        while (st.hasMoreTokens()) {
                    ...
                    if (token.equalsIgnoreCase("regex_fail_file")) {
                        ...
                        if (!new File(fn).isAbsolute()) {
                            fn = scache.cfgdir + File.separator + fn;
                        }
                        regex_fail = parseRealRegexpFile(fn, regex_fail);
                    ...
                    if (token.equalsIgnoreCase("no_cache")) {
                        while (st.hasMoreTokens()) {
                    ...
                    if (token.equalsIgnoreCase("cacheonly")) {
                        while (st.hasMoreTokens()) {
                            nocache = addRegexpToArray(st.nextToken(), nocache, true);
                        }
                        cacheonly = true;
                    ...
                    if (token.equalsIgnoreCase("ui_searchcache_factor")) {
                        ui.searchcachefactor = Float.valueOf(st.nextToken()).floatValue();
                        continue;
                    }

                    if (token.equalsIgnoreCase("ui_searchcache_minage")) {
                        ui.minsearchcachettl = garbage.timestring(st.nextToken("\r\n"));
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("quick_abort_pct")) {
                        ...
                        if (request.qa_percent > 1) {
                            request.qa_percent /= 100f;
                        }
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("errordocument")) {
                        ...
                        if (code == 403) {
                            custom403 = st.nextToken();
                        } else if (code == 400 || code == 500) {
                            cacheobject.custom500 = st.nextToken();
                        } else {
                    ...
                    if (token.equalsIgnoreCase("overwrite_by_redirect")) {
                        cacheobject.overwrite_by_redirect = getYesNo(st.nextToken());
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("trace_timestamp")) {
                        httpreq.trace_timestamp = getYesNo(st.nextToken());
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("clear_flags_on_shutdown")) {
                        clear_flags_on_shutdown = getYesNo(st.nextToken());
                        continue;
                    }
                    ...
                    if (token.equalsIgnoreCase("default_refresh_pattern")) {
                        ...
                        if (reload_age > min_age) {
                            reload_age = min_age / 2;
                        }
                        if (max_age <= min_age) {
                            max_age = min_age + 1000L * 60L * 60L * 24 * 3;
                        }
                        expire_age = garbage.timestring(st.nextToken());
                        redir_age = garbage.timestring(st.nextToken());
                        if (expire_age > max_age) {
                            expire_age = max_age;
                        }
                        continue;
                    }
                    if (token.equalsIgnoreCase("refresh_pattern")) {
                        ...
                        if (xreload_age > xmin_age) {
                            xreload_age = xmin_age / 4;
                        }
                        if (xmax_age <= xmin_age) {
                            xmax_age = xmin_age + 1000L * 60L * 60L * 24 * 4;
                        }
                        long xexpire_age, xredir_age;
                        xexpire_age = garbage.timestring(st.nextToken());
                        xredir_age = garbage.timestring(st.nextToken());
                        if (xexpire_age > xmax_age) {
                            xexpire_age = xmax_age;
                        }
                        /* pridat do pole */
        ...
        if (httpreq.logpatterns != null) {
            httpreq.logfilez = new DataOutputStream[httpreq.logpatterns.length];
        }

        if (swap_level1_dirs < 1) {
            swap_level1_dirs = 1;
        }
        if (swap_level2_dirs < 1) {
            swap_level2_dirs = 1;
        }
        ...
        try {
            if (allowed == false) {
                if (regex_fail != null) {
                    ...
                    ln--;
                    for (int i = regex_fail.length - 1; i >= 0; i--) {
                        if (regex_fail[i].match(str, 0, ln) != null) {
                            if (trace_fail) {
 ...
 /* Mascot of Commodore C64 is Captain Hook, C64 created piracy */
        if (dmachine_mask != null) {
            if (!uireq) {
                dmachine:
                for (int i = dmachine_mask.length - 1; i >= 0; i--) {
                    if (dmachine_mask[i].matches(requested)) {
                        ...
                        synchronized (this) {
                            Hashtable<String, String> dmq = ui.load_dm_queue();
            ...
            if (allow_cookies_to != null && request.allow_all_session_cookies == false) {
                ...
                while (true) {
                    for (int i = allow_cookies_to.length - 1; i >= 0; i--) {
                        if (host.endsWith(allow_cookies_to[i])) {
                            break cookieloop;
                        }
                    }
        ...
        if (!ftp) {
            ...
            if (direct) {
                ...
                try {
                    ...
                    req.setTarget(null, 80);
                } catch (NumberFormatException e) {
                    req.send_error(400, "Port is not a numeric constant: '" + parsed[1] + "'");
                }
                ...
                if (http_proxy_auth != null) {
                    req.add_header(http_proxy_auth);
                }
            }
        } else {
            /* sent request to FTP proxy */
            req.setTarget(ftp_proxy, ftp_proxy_port);
            if (ftp_proxy_auth != null) {
                req.add_header(ftp_proxy_auth);
            }
        }
        ...
        switch (req.method) {
            ...
            case httpreq.REQUEST_CONNECT: {
                ...
                if (allowconnect == null) {
                    req.send_error(501, "CONNECCT method is disabled by configuration.");
                }
                if (https_proxy == null) {
                    direct = true;
                }
    ...
    final public void run() {
        ...
        while (true) {
            ...
            if (checkFlag(immediate_shutdownflag, clear_flags_on_shutdown)) {
                checkFlag(aliveflag, true);
                Enumeration<cachedir> e2 = dircache.elements();
                ...
                httpreq.flush(httpreq.logfilez);
                System.out.println(new Date() + " Server stopped by Immediate shutdown flag.");
                System.exit(0);
            }
            ...
            if (regex_fail_filename != null) {
                if (new File(regex_fail_filename).lastModified() != regex_fail_timestamp) {
                    if (loglevel > 2) {
                        System.out.println(new Date() + " Reloading regex_fail_file " + regex_fail_filename);
                    }
                    regex_fail = parseRealRegexpFile(regex_fail_filename, null);
            ...
            if (redir_filename != null) {
                if (new File(redir_filename).lastModified() != redir_timestamp) {
                    if (loglevel > 2) {
                        System.out.println(new Date() + " Reloading redirect_file " + redir_filename);
                    }
            ...
            if (remap_filename != null) {
                if (new File(remap_filename).lastModified() != remap_timestamp) {
                    if (loglevel > 2) {
                        System.out.println(new Date() + " Reloading remap_file " + remap_filename);
                    }
    ...
    private synchronized void save() {
        ...
        int gc = 0;
        int cleaned = 0;
        Enumeration<String> e1 = dircache.keys();
        Enumeration<cachedir> e2 = dircache.elements();
        ...
        if (stat_log != null) {
            try {
                dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(stat_log, true), 4096));
        ...
        if (saved > 0) // write hit stats
        {
            ...
            sb.append(new Date());
            sb.append(" - ");
            sb.append((int) (100.0f * (cacheobject.c_hit + cacheobject.c_block)
        ...
        if (cacheobject.b_miss + cacheobject.b_hit > 0) {
            ...
            sb.append(new Date());
            sb.append(" - ");
            sb.append((int) (100.0f * cacheobject.b_hit / ((float) cacheobject.b_miss + cacheobject.b_hit)));
            sb.append("% BYTES: ");
        ...
        if (saved == 0) {
            ...
            dircache = new Hashtable<String, cachedir>(40); // reinit hashtable

            if (checkFlag(shutdownflag, clear_flags_on_shutdown)) {
...
urldir = /ddfgfds/rerew/ - musi koncit s /
 ...
 /* zacneme budovat cestu */
        ...
        if (File.separatorChar != '/') {
            urldir = urldir.replace('/', File.separatorChar);
        }
 ...
 /* nahrazujeme nepratelske znaky */
    ...
    static regexp[] addRegexpToArray(String what, regexp[] array, boolean clean) {
        ...
        if (clean) {
            array = cleanUpRegexpArray(array, what);
        }
    ...
    static gnu.rex.Rex[] addRealRegexpToArray(String what, gnu.rex.Rex[] array) {
        ...
        if (array == null) {
            array = new gnu.rex.Rex[1];
            try {
                array[0] = gnu.rex.Rex.build(what);
            } catch (gnu.rex.RegExprSyntaxException errr) {
                System.out.println("Regexp compilation failed: " + what);
                return null;
            }
            return array;
        }
        ...
        gnu.rex.Rex[] tmp;
        gnu.rex.Rex whatrule;
        try {
            whatrule = gnu.rex.Rex.build(what);

            tmp = new gnu.rex.Rex[array.length + 1];
            System.arraycopy(array, 0, tmp, 0, array.length);
            tmp[array.length] = whatrule;
            return tmp;
        } catch (gnu.rex.RegExprSyntaxException errr) {
 ...
 /* hvezdicka muze byt jen na konci retezce */
    public static String simpleWildMatch(String mask, String test) {
        ...
        int ml = mask.length();
        if (i != ml - 1) {
            throw new IllegalArgumentException("* is not at end of string " + mask);
        }

        if (test.regionMatches(0, mask, 0, ml - 1)) {
            if (tl >= ml) {
                return test.substring(ml - 1);
            } else {
                return "";
            }
        }
    ...
    static gnu.rex.Rex[] parseRealRegexpFile(String fname, gnu.rex.Rex[] oldf) {
        ...
        try {
            ...
            while ((line = dis.readLine()) != null) {
                ...
                oldf = addRealRegexpToArray(token, oldf);
    ...
    static regexp[] parseRegexpFile(String fname, regexp fail[]) {
        try {
            ...
            while ((line = dis.readLine()) != null) {
                ...
                token = st.nextToken();
                if (token.indexOf('*') == -1
                        && token.indexOf('/') == -1) /* domain name only support */ {
                    token = "*" + token + "/*";
                }
 ...
 * 3.   hostname or ip address / number of bits
    ...
    public static boolean checkInetAdr(byte[] adr) {
        ...
        nextadres:
        for (int i = httpreq.allowed.length - 1; i >= 0; i--) {
