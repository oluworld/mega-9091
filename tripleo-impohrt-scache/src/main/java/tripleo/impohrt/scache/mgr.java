			...
			while ((line = dis.readLine()) != null) {
				...
				try {
					...
					if (token.equalsIgnoreCase("default_refresh_pattern")) {
						...
						if (max_age <= min_age) max_age = min_age + 1000L * 60L * 60L * 24 * 3;
					...
					if (token.equalsIgnoreCase("refresh_pattern")) {
						...
						if (xmax_age <= xmin_age) xmax_age = xmin_age + 1000L * 60L * 60L * 24 * 4;
	...
	final public void process_request(request req) throws MalformedURLException, IOException {
		...
		if (!ftp) {
			...
			if (direct) {
				...
				} catch (UnknownHostException e) { req.setTarget(null, 80); } catch (NumberFormatException e) {
...
/* Ulozi adresare cachovane v pameti na disk a udela gc, flush logy */
	private synchronized void save() {
		...
		/* close stat_logfile */
		if (dos != null)
			try { dos.close();} catch (IOException err) {
				stat_log = null;
			}
			...
			while ((line = dis.readLine()) != null) {
				...
				while (st.hasMoreTokens()) {
					...
					if (token.indexOf('*') > -1) {
						...
						continue;
					} else if (token.equalsIgnoreCase("all")) { return null;}
