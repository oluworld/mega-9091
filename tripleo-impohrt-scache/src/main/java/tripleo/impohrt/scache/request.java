...
public final class request {
    ...
    /* wants/gets client side keepalive? */
    ...
    /* konstruktor */
    request(int htver, int met, String url, Vector<String> head, DataInputStream in, DataOutputStream out) throws MalformedURLException {
        hasCookie = false;
        ims = httprc = 0;
        ctsize = -1;
        reload = false;
 ...
 /* projdeme hlavicky, zajima nas Pragma: no-cache
        ...
        if (proxyvia && !hasVia) {
            headers.addElement("Via:" + via_header);
        }
        /* pro jistotu */
        ...
        /* - F O R W A R D E R - */
        if (url.charAt(0) == '/') {
            throw new MalformedURLException("Forwarding is currently disabled due to infinite loop bug");

            /*
			// start forwarder engine !
			protocol="http";
        ...
        if (met != httpreq.REQUEST_CONNECT) {
            ...
            if (pos == -1) {
                throw new MalformedURLException("Can not extract protocol from URL");
            }
 ...
 /*   @    - ok      */
                ...
                throw new MalformedURLException("Incorrect auth syntax in URL");
            } // So no need to check index out of bounds
        } else {
            protocol = "https";
            cacheable = false;
        }
    ...
    final public void rewriteURL(String newURL) {
        this.URL = newURL;
        if (!host && referer_hack < 2) {
            return; // scan and kill old host header
        }
        String s, s1;
        int cachedsize = headers.size();
    ...
    final public void removeOutgoingCookies() {
        ...
        for (int i = 0; i < cachedsize; i++) {
            ...
            if (s1.equals("cookie")) {
                if (trace_cookie) {
                    System.out.println("[TRACE " + Thread.currentThread().getName() + "] @ Unallowed outgoing cookie!");
                }
                if (wafer == null) {
                    headers.removeElementAt(i--);
                    cachedsize--;
    ...
    /* header string must be in lower case */
    final public String getHeader(String header) {
        String s, s1;
        int cachedsize = headers.size();
    ...
    final public void addHost(String hostname, String port) {
        ...
        if (port == null) {
            headers.addElement("Host: " + hostname);
        } else {
    ...
    private int get_and_update_maxforwards() {
        ...
        for (int i = 0; i < cachedsize; i++) {
            ...
            if (s1.equals("max-forwards")) {
                ...
                if (maxforwards > 0) {
                    headers.setElementAt("Max-Forwards: " + (maxforwards - 1), i);
                }
                return maxforwards;
            }
        }
 ...
 /* pokud nemame ctsize, tak cteme az na konec */
    ...
    final public void run_request(WebConnection wc) throws IOException {
        ...
        if (retry) {
            ...
            } else if (mgr.loglevel > 1) {
                System.out.println(Thread.currentThread().getName() + " No response from server " + getURL() + ", will retry.");
            }
    ...
    final public void raw_send_request(WebConnection wc) throws IOException {
        ...
        for (int i = 0; i < hscache; i++) {
            sout.writeBytes(headers.elementAt(i));
            sout.writeBytes("\r\n");
        }
        sout.writeBytes("\r\n");
        sout.flush();
 ...
 /* need to restart reader on 100-Continue */
        ...
        if (ctsize == -1 && httprc != 304 && httprc != 204) {
            keepalive = false;
        }
        ...
        if (ctsize > maxcacheable) {
            cacheable = false;
        } else if (method != httpreq.REQUEST_GET) {
            cacheable = false;
        ...
        if (dmachine_ctype_mask != null && httprc != 304) {
            if (ctsize == -1 || (ctsize > 0 && ctsize > dmachine_ctype_min)) {
                dmachine:
                for (int i = dmachine_ctype_mask.length - 1; i >= 0; i--) {
        ...
        if ((httprc == 301 || httprc == 300) && exp == 0) {
            exp = System.currentTimeMillis() + mgr.max_age / 2;
        }
    ...
    final public void transfer_object(WebConnection wc, OutputStream file, cachedir dir) throws IOException {
        ...
        int todo;
        if (wc.socket != null) {
            wc.socket.setSoTimeout(read_timeout);
        }
        if (chunked == true) {
            ctsize = -1;  // drop ctsize on chunked streams
        }
        if (ctsize < 0) {
            ...
            while (todo > 0) {
                ...
                try {
                    if (!clientdead) {
                        out.write(b, 0, i);
                        log_size += i;
                    } else if (qa_minlen < 0 && System.currentTimeMillis() - whenaborted > qa_maxtime) {
                        ctsize = read;
                        wc.close();
        ...
        if (filedead) {
            throw new IOException("Write error (disk full?)");
        }
        ctsize = read;
    }
    ...
    final public void add_ims(long when) {
        if (etag != null) {
            headers.insertElementAt("If-None-Match: " + etag, 1);
        }
        if (when == 0) {
            return;
        }
 ...
 /* nereportuje io chyby */
    final public void send_headers() {
        ...
        try {
            ...
            for (int i = 0; i < headerscache; i++) {
                out.writeBytes(headers.elementAt(i));
                out.writeBytes("\r\n");
            }
            out.writeBytes("\r\n");
            out.flush();
    ...
    final public void make_headers(int rc, String ctype, String enc, String loc, int sz, long lm, long exp, String et) {
        ...
        Date d = new Date();
        if (ctype != null) {
            headers.addElement("Content-Type: " + ctype);
        }
        if (enc != null) {
            headers.addElement("Content-Encoding: " + enc);
        }
        if (sz >= 0) {
        ...
        if (lm != 0) {
            headers.addElement("Last-Modified: " + printDate(new Date(lm)));
        }

        if (et != null) {
            headers.addElement("ETag: " + et);
        }
        /*
         ...
         */
        if (exp > 0) {
            headers.addElement("Expires: " + printDate(new Date(exp)));
        }
        if (loc != null) {
            headers.addElement("Location: " + loc);
        }
