		...
		*/
		...
		headers:for (i = 0; i < cachedsize; i++) {
			...
			if (s1.equals("content-length"))
				...
				} catch (Exception ignore) {} finally { continue;}
		...
		if (met != httpreq.REQUEST_CONNECT) {
			...
			if (pos == -1) throw new MalformedURLException("Can not extract protocol from URL");
...
/* failsafe send request and read+parse reply headers */
	final public void run_request(WebConnection wc) throws IOException {
		...
		if (retry) {
			...
			} else if (mgr.loglevel > 1) System.out.println(Thread.currentThread().getName() + " No response from server " + getURL() + ", will retry.");
...
/* reads headers from server and parses it */
	final public void read_headers(String firstline, WebConnection wc) throws IOException {
		...
		} catch (Exception http09) { /* 502 - Bad gateway */
			wc.close();
		...
		while (true) {
			...
			if (s1.equals("last-modified")) {
				try {
					ims = new Date(s2).getTime();
				} catch (IllegalArgumentException e) { ims = 0;}
				continue;
			}
			...
			if (s1.equals("expires")) {
				try {
					...
					if (exp == 0) throw new IllegalArgumentException("Forced parse error");
		...
		if (ctsize > maxcacheable)
			cacheable = false;
		else if (method != httpreq.REQUEST_GET) cacheable = false; /* CACHE ONLY GET REQUESTS */
		...
		if (exp != 0) /* Check Expire: against Date: header */ {
			...
			for (int i = headers.size() - 1; i >= 0; i--) {
				...
				if (s1.equals("date")) {
					...
					} catch (IllegalArgumentException e) { now = 0;}
					break;
				}
			} /* Date: header hunting */
			if (now == 0) now = System.currentTimeMillis();
			if (exp <= now) {  /* Expire date NOW or in the past is the same as Pragma: no-cache */
			...
			} /* exp<=now, killed */ else { /* fix expire to out local time */
...
/* nereportuje io chyby */
	...
	final public void make_headers(int rc, String ctype, String enc, String loc, int sz, long lm, long exp, String et) {
		...
		} else if (rc != 304) keepalive = false;
