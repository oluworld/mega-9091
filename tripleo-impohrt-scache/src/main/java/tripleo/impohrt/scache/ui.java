...
public class ui implements Runnable {
    ...
    public static long minsearchcachettl = 30 * 1000L;

    public static final int OFF = 0;
    public static final int ON = 1;
    public static final int ASK = 2;

    public static final int NAME_LEN = 42;
    ...
    public static String process(String URL) {
        if (URL.startsWith("/loader/setdepth?")) {
        ...
        } else if (URL.startsWith("/loader/confirm/add?")) {
            String ref = getparam(URL, "ref");
            ...
            return "<title>Smart Cache: Confirm adding URL</title>\n<body bgcolor=\"#ffffe5\" text=\"#000000\">You have requested a URL: <em><a href=\"" + URL + "\">" + URL + "</a></em> which is not in cache and can not be downloaded from remote server now. Do you want to add this URL to Smart Cache Loader queue for later batch retrieve?<p>\n"
                    ...
                    + "Referer: <input type=text size=40 name=ref" + (ref != null ? " value=\"" + ref + "\"" : "") + "><br>\n"
        ...
        } else if (URL.startsWith("/loader/confirm/refresh?")) {
            URL = getparam(URL, "url");
        ...
        } else if (URL.startsWith("/loader/add?") || URL.startsWith("/loader/refresh?")) {
            ...
            synchronized (LOCK) {
                Hashtable<String, String> q = load_ldr_queue();
                q.put(url, (depth != null ? "depth=" + depth + " " : "") + (ref != null ? "ref=" + ref + " " : "") + options);
                try {
                    save_ldr_queue(q);
            ...
            for (Enumeration<String> en = q.keys(); en.hasMoreElements();) {
                URL = en.nextElement();
        ...
        } else if (URL.startsWith("/loader/queue/delete?")) {
            String url = getparam(URL, "url");
            if (url == null) {
            ...
            synchronized (LOCK) {
                ...
                try {
                    save_ldr_queue(q);
                    return "<title>Delete URL from queue</title>\nRequested URL <a href=\"" + url + "\">" + url + "</a> has been removed from queue (if exists there).<p><a href=\"/loader/queue/show\">Show queue</a>.\n<p><a href=\"/\">Back to home</a>.\n";
                } catch (IOException e) {
            ...
            result.append("<p>End of queue.\n\n<p><a href=\"/\">Back to home</a>.\n");
            return result.toString();
        } else if (URL.startsWith("/dmachine/queue/delete?")) {
            String url = getparam(URL, "url");
            if (url == null) {
                return "Can not delete null URL from queue!";
            }
            synchronized (LOCK) {
                Hashtable<String, String> q = load_dm_queue();
                q.remove(url);
                try {
                    save_dm_queue(q);
        ...
        } else if (URL.startsWith("/store/browse?")) {
            boolean del = false;
            String bURL;
            ...
            if (URL.length() == 0 || !f.isDirectory()) {
                ...
                for (Enumeration<cacheobject> en = d.getObjects(); en.hasMoreElements();) {
                    cacheobject o;
                    o = en.nextElement();
                    if (o.needSave() == false) {
                        continue;
            ...
            return ans.toString();
        } else if (URL.startsWith("/store/search?")) {
            String depth = getparam(URL, "depth");
            if (depth == null) {
                depth = "2";
            }
            ...
            return ans.toString();
        } else if (URL.startsWith("/store/delete?")) {
            URL = getparam(URL, "url");
            if (URL == null) {
        ...
        } else if (URL.startsWith("/store/rmdir?")) {
            String bURL;
            URL = getparam(URL, "dir");
            if (URL == null) {
                return "Missing dir argument!";
            }
            if (URL.indexOf("..") > -1) {
                return "Unsecure directory name.";
            }
        ...
        } else if (URL.startsWith("/store/purgesearchcache")) {
        ...
        } else if (URL.startsWith("/ui/refresh?off")) {
            ...
            display_trace_switch("Trace timestamps", ans, httpreq.trace_timestamp, 12);
            ans.append("</table>\n<p><a href=\"/\">Back to home</a>.");
            return ans.toString();
        } else if (URL.startsWith("/trace/set")) {
            String what;
            what = getparam(URL, "what");
            if (what == null) {
                return "Missing what argument!";
            }
            int i = 0;
            try {
            ...
            if (q < -1) {
                q = (int) (Math.random() / (1f / (float) scache.quotes.length));
            }
            if (q < 0) {
                q = 0;
            } else if (q >= scache.quotes.length) {
                q = scache.quotes.length - 1;
            }
        ...
        } else if (URL.startsWith("/logs/console?")) {
            URL = getparam(URL, "loglevel");
            if (URL == null) {
                return "Error! Required parameter 'loglevel' was not found.";
            }
            int lvl = 3;
            try {
 ...
 /* /2/3/www.lakret.cz_2345/ -> http://www.lakret.cz:2345/ */
    ...
    static Hashtable<String, String> load_ldr_queue() {
        ...
        StringTokenizer st;
        try {
            DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(loader_queue_file), 4096));
    ...
    static void save_ldr_queue(Hashtable<String, String> data) throws IOException {
        ...
        for (Enumeration<String> keys = data.keys(); keys.hasMoreElements();) {
            URL = keys.nextElement();

            dos.write((data.get(URL)).getBytes());
            dos.write(' ');
    ...
    static Hashtable<String, String> load_dm_queue() {
        ...
        StringTokenizer st;
        try {
            DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(mgr.dmachine_queue)));
            while ((line = dis.readLine()) != null) {
                st = new StringTokenizer(line);
                i = st.countTokens();
                if (i != 1) {
 ...
 /* vygeneruje Vector() skladajici se z adresaru o dane hloubce */
    ...
    public static void search_savecache(Vector<String> cache, int depth, long scan) {
        try {
            ...
            Enumeration<String> e = cache.elements();
            while (e.hasMoreElements()) {
                dos.writeUTF(e.nextElement());
            }
            dos.writeUTF("");
            dos.close();
    ...
    public static Vector<String> search_loadcache(int depth) {
        ...
        v = null;
        f = new File(mgr.cache_dir, SEARCHCACHE + depth);
        if (!f.exists()) {
            return null;
        }

        try {
            DataInputStream din = new DataInputStream(new BufferedInputStream((f.fis()), 4096));
            ...
            if (filetime + minsearchcachettl < now) {
                if (filetime + scantime * searchcachefactor < now) {
