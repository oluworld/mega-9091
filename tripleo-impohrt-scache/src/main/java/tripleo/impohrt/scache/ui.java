...
public class ui implements Runnable {
		...
		while (true) {
			...
			if (clients.activeCount() > scache.MAXHTTPCLIENTS)
				try {
					...
					clientSocket.close();
				} catch (IOException e) {} finally { continue;}
	...
	public final static String process(String URL) {
		if (URL.startsWith("/loader/setdepth?")) {
			...
			else if (depth.length() == 0) depth = null;
			...
			if (q < 0)
				q = 0;
			else if (q >= scache.quotes.length) q = scache.quotes.length - 1;
...
/* /2/3/www.lakret.cz_2345/ -> http://www.lakret.cz:2345/ */
	...
	final static Hashtable<String,String> load_dm_queue() {
		...
		try {
			...
			dis.close();
		} catch (IOException grr) {} catch (NullPointerException uch) {}
