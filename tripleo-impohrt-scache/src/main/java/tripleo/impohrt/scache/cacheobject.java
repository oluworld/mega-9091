...
public final class cacheobject {
    ...
    public static final int REDIRSTRLEN = REDIRSTR.length();
    /* statistics */
    ...
    public static boolean overwrite_by_redirect;
    ...
    final public void make_request(request r, long reload_age, long min_age, long max_age, float percent, long expire_age, long redir_age) throws IOException {
        ...
        /* test na expiraci */
        if (needRefresh(r, reload_age, min_age, max_age, percent, expire_age, redir_age)) {
            refresh_object(r);
            return;
        }
    ...
    final public void compress(int level) {
        ...
        if (size <= auto_compress) {
            return;
            /* waste of CPU cycles */
        }
        if (nocompress != null) {
            for (int z = nocompress.length - 1; z >= 0; z--) {
                if (localname.toLowerCase().endsWith(nocompress[z])) {
                    return;
                }
            }
        }
        ...
        int i;
        try {
            GZIPOutputStream gz = new GZIPOutputStream(t.fos(), 8192);
            DataInputStream sin = new DataInputStream(new BufferedInputStream(new File(dir.getLocalDir() + localname).fis(), 4096));
            ...
            if (!rename(t, n)) {
                if (mgr.loglevel > 0) {
                    System.out.println(Thread.currentThread().getName() + " COMPRESS ERROR: Rename " + t + " to " + n + " failed.");
                }
                dir.dirty = true;
                return;
            }
            ...
            dir.dirty = true;
            if (mgr.loglevel > 3) {
                System.out.println("Compressed: " + dir.getLocalDir() + localname + " (saved=" + (size - sz) + " B, ratio=" + (int) (100 * (1.0f - (float) sz / size)) + "%)");
            }
            size = sz;
        } catch (IOException j) {
            if (mgr.loglevel > 0) {
                System.out.println(Thread.currentThread().getName() + " COMPRESS ERROR: " + j);
            }
        }
    }
 ...
 /* aby se to nemotalo do jmena adresare */
        if (httprc == 301 || httprc == 302 || httprc == 303 || httprc == 307) {
            tmp += "_r";
        }
    ...
    private void load_object(request r) throws IOException {
        ...
        if (localname == null) {
            localname = RESERVED;
            /* prevents from async dir cleaning */
        }
        if (ctype == null) {
        ...
        try {
            ...
            if (dmachine_offmask != null) {
                dmachine:
                for (int i = dmachine_offmask.length - 1; i >= 0; i--) {
            ...
            if (ui.loader_add_missing > 0) {
                /* redir to UI */
                r.make_headers(302, "text/html", null, "http://" + ui.ui_hostname + ":" + ui.uiport + "/loader/" + (ui.loader_add_missing == ui.ASK ? "confirm/" : "") + "add?url=" + ui.escape(dir.URLbase + name) + "&ref=" + ui.escape(r.getHeader("referer")), REDIRSTRLEN, 99999999, 199999999, null);
                r.headers.addElement("Pragma: no-cache");
                r.send_headers();
                r.sendString(REDIRSTR);
            } else /* CUSTOM 500 RC komedie */ if (custom500 == null) {
                r.send_error(500, "Proxy load failed (" + ccc + ") and object not found in cache");
            } else if (custom500.charAt(0) == '0') {
                if (name.endsWith(".js")) {
                ...
                r.make_headers(301, null, null, custom500, 0, new Date().getTime(), 0, null);
                r.send_headers();
            }
            return;
        }
        ...
        try {
            ...
            dir.dirty = true;
            if (mgr.loglevel > 1) {
                System.out.println("LOAD FAILED: " + dir.getLocalDir() + name + " (" + e + ")");
            }
            throw e;
        }
        r.send_headers();
 ...
 /* bad RC */
        ...
        try {
            if (f != null) {
                fout = new DataOutputStream(new BufferedOutputStream((f.fos()), 4096));
            } else {
                /* not caching */
                fout = null;
                if (localname != null && localname.equals(RESERVED)) {
        ...
        try {
            ...
            dir.dirty = true;
            if (mgr.loglevel > 1) {
                System.out.println(Thread.currentThread().getName() + " LOAD FAILED: " + dir.getLocalDir() + name + " (" + e + ")");
            }
            throw e;
        } finally {
            b_miss += r.ctsize;
        ...
        if (!rename(f, n)) {
            if (mgr.loglevel > 0) {
                System.out.println(Thread.currentThread().getName() + " LOAD ERROR: Rename " + f + " to " + n + " failed.");
            }
            if (localname != null && localname.equals(RESERVED)) {
                localname = null;
            }
    ...
    private void refresh_object(request r) throws IOException {
        ...
        try {
            ...
            if (r.reload && ui.loader_add_reloads > 0) {
                /* redir to UI */
                r.make_headers(302, "text/html", null, "http://" + ui.ui_hostname + ":" + ui.uiport + "/loader/" + (ui.loader_add_reloads == ui.ASK ? "confirm/" : "") + "refresh?url=" + ui.escape(dir.URLbase + name), REDIRSTRLEN, 0, 199999999, null);
                r.headers.addElement("Pragma: no-cache");
                r.headers.addElement("Cache-Control: no-store");
                r.send_headers();
                r.sendString(REDIRSTR);
            } else {
                send_fromcache(r);
 ...
 /* mel tu samou jako my, protoze IMS delame na nas objekt */
        ...
        if (rc == 304) {
            ...
            ConnectionHandler.releaseConnection(wc);
            if (r.exp > 0) {
                expires = r.exp; // refresh possible Expires for Microsoft IIS 4.0
            }
            if (r.etag != null) {
                etag = r.etag;   // update Etag info
            }
            if (mgr.loglevel > 3) {
                System.out.println("Checked: " + dir.getLocalDir() + localname);
            }
            if (clims > 0) {
                /* musime poslat objekt z cache */
                r.ims = 0;
                r.etag = null;
                r.chunked = false;
                send_fromcache(r);
        ...
        // check for overwriting a good page with redirect
        if (overwrite_by_redirect == false) {
            if (location == null && r.location != null) {
                r.nocache();
            }
        }
 ...
 /* bad RC */
        ...
        try {
            ...
            if (mgr.loglevel > 1) {
                System.out.println("REFRESH FAILED: " + dir.getLocalDir() + localname + " (" + e + ")");
            }
            throw e;
        } finally {
            b_miss += r.ctsize;
        ...
        if (!rename(f, n)) {
            if (mgr.loglevel > 0) {
                System.out.println(Thread.currentThread().getName() + " REFRESH ERROR: Rename " + f + " to " + n + " failed.");
            }
            dir.dirty = true;
            return;
        }
        ...
        if (mgr.loglevel > 3) {
            System.out.println("Refreshed: " + n + " (delta=" + printDelta(delta) + ", size=" + size + " B)");
        }
        if (auto_compress > 0) {
            compress(9);
        }
    ...
    private void send_fromcache(request r) throws IOException {
        ...
        if (encoding != null && auto_decompress > 0 && ctype.startsWith("text/")) {
            if (r.encoding == null
                    || // client do not supports gziped data
        ...
        if (generate_lastmod) {
            if (decomp) {
                r.make_headers(httprc, ctype, null, location, -1, lastmod == 0 ? date : lastmod, expires, etag);
            } else {
    ...
    private boolean needRefresh(request r, long reload_age, long min_age, long max_age, float percent, long expire_age, long redir_age) {
        ...
        if (lastmod == 0) {
            ...
            return true;
            /* usetrime si nejake to pocitani */
        }
    ...
    /* renames object name and updates directory information */
    ...
    public static String printDelta(long delta) {
        ...
        if (mdelta > 60 * 24 * 7 * 4 * 12 * 2) {
            return (mdelta / 60 / 24 / 365) + "Y";
        } else if (mdelta > 60 * 24 * 7 * 12) {
            return (mdelta / 60 / 24 / 30) + "M";
        } else if (mdelta > 60 * 24 * 5 * 7) {
            return (mdelta / 60 / 24 / 7) + "w";
        } else if (mdelta > 60 * 72) {
            return (mdelta / 60 / 24) + "d";
        } else if (mdelta > 200) {
            return (mdelta / 60) + "h";
        } else if (delta < 60000) {
            return (delta / 1000) + "s";
