...
/* P R I V A T E    P R O P E R T Y */
	...
	final public void compress(int level) {
		...
		try {
			...
			/* rename to new file */
			if (!rename(t, n)) {
				if (mgr.loglevel > 0) System.out.println(Thread.currentThread().getName() + " COMPRESS ERROR: Rename " + t + " to " + n + " failed.");
				dir.dirty = true;
				return;
			}
			...
			dir.dirty = true;
			if (mgr.loglevel > 3) System.out.println("Compressed: " + dir.getLocalDir() + localname + " (saved=" + (size - sz) + " B, ratio=" + (int) (100 * (1.0f - (float) sz / size)) + "%)");
			size = sz;
		} catch (IOException j) {
			if (mgr.loglevel > 0) System.out.println(Thread.currentThread().getName() + " COMPRESS ERROR: " + j);
		}
	}
...
/* L O A D ! */
	...
	private void load_object(request r) throws IOException {
		...
		try {
			...
			/* CUSTOM 500 RC komedie */ if (custom500 == null)
		...
		} catch (IOException e) { /* tmp file create error */
			fout = null;
			if (localname != null && localname.equals(RESERVED)) localname = null;
			dir.dirty = true;
		}
		...
		if (!rename(f, n)) {
			if (mgr.loglevel > 0) System.out.println(Thread.currentThread().getName() + " LOAD ERROR: Rename " + f + " to " + n + " failed.");
			if (localname != null && localname.equals(RESERVED)) {
				localname = null;
			}
			dir.dirty = true;
