...
public final class scache {
	...
	public final static String CACHEVER = "0.93";
...
/* Sri Isopanisad -> http://www.iskcon.net/isopanisad/ */
	...
	final public static void main(String argv[]) {
		...
		System.err.println(VERSION + " - full featured caching proxy server" + /* " and web forwarder"*/ ".\n" +
		        "Copyright (c) Radim Kolar 1998-2005. OpenSource software; There is NO warranty.\n" +
		...
		for (int i = 0; i < argv.length; i++) {
			if (argv[i].equals("-cfgdir")) {
			...
			} else if (argv[i].equals("-killunref")) { ACT |= ACT_KILLUNREF | ACT_EXIT;} else if (argv[i].equals("-ro") || argv[i].equals("-cdrom") ||
			...
			} else if (argv[i].equals("-importcache")) {
				if (i + 1 < argv.length && !argv[i + 1].startsWith("-")) {
					if (filesystemchecked == false) {
						m.check_filesystem();
						filesystemchecked = true;
				...
				if (i + 2 < argv.length && !argv[i + 1].startsWith("-")
					...
					if (argv[i].indexOf("full") > 0)
						type = garbage.EXPORT_DATE;
					else if (argv[i].indexOf("lru") > 0) type = garbage.EXPORT_LRU;
			...
			} else if (argv[i].equals("-cachedir") ||
				...
				if (i + 1 < argv.length && !argv[i + 1].startsWith("-")) {
					...
					System.out.println("[ERROR] -cachedir <Directory> [swap_level1_dirs] [swap_level2_dirs]");
					continue;
				}
				if (i + 1 < argv.length && !argv[i + 1].startsWith("-")) {
					try {
						...
						i++;
					} catch (NumberFormatException nse) {continue;}
				}
				if (i + 1 < argv.length && !argv[i + 1].startsWith("-")) {
					try {
						int z = Integer.valueOf(argv[i + 1]).intValue();
			...
			} else if (argv[i].equals("-rebalance")) { ACT |= ACT_REBALANCE | ACT_EXIT;} else if (argv[i].equals("-fakegc")) {
				ACT |= ACT_FAKEGC | ACT_EXIT;
			} else if (argv[i].equals("-faststart")) { faststart = true;} else if (argv[i].equals("-version")) {
				System.out.println(CACHEVER);
				System.exit(0);
			} else if (argv[i].equals("-local")) {
				ACT |= ACT_LOCAL;
			} else if (argv[i].equals("-nocache")) {
				ACT |= ACT_NOCACHE;
			} else if (argv[i].equals("-direct")) {
				m.http_proxy = null;
				m.no_proxy = null;
				m.https_proxy = null;
				System.err.println("[INFO] Bypassing parent proxy.");
			} else if (argv[i].equals("-nofail")) {
				m.fail = null;
				m.pass = null;
				m.regex_fail = null;
			...
			} else if (argv[i].equals("-fastrefresh")) {
				m.reload_age = 0;
				m.min_age = 0;
			...
			} else if (argv[i].equals("-http_proxy")) {
				if (i + 2 < argv.length && (!argv[i + 1].startsWith("-")) && (!argv[i + 2].startsWith("-")))
					...
					System.out.println("[ERROR] -http_proxy <Hostname> <Port>");
			} else if (argv[i].equals("-ftp_proxy")) {
				if (i + 2 < argv.length && (!argv[i + 1].startsWith("-")) && (!argv[i + 2].startsWith("-")))
					try {
					...
					System.out.println("[ERROR] -ftp_proxy <Hostname> <Port>");
			} else if (argv[i].equals("-repair")) {
				ACT |= ACT_REPAIR | ACT_EXIT;
			} else {
				if (!argv[i].equals("-?") &&
				        !argv[i].equals("-h") &&
				        !argv[i].equals("-help"))
		...
		while (ACT > 0) {
			if ((ACT & ACT_REBALANCE) > 0) {
				...
				try {
					...
					m.lmfactor = 99999;
					m.no_proxy = null;
					m.http_proxy_port = 1; // I HOPE THAT THIS PORT NUMBER IS UNUSED
					m.https_proxy = m.http_proxy;
					m.https_proxy_port = m.http_proxy_port;
					System.err.println("Smart Cache set to FORCED LOCAL MODE. Will not connect to ANY (localhost included) site.");
				} catch (java.net.UnknownHostException e) {}
				ACT -= ACT_LOCAL;
			} else if ((ACT & ACT_EXIT) > 0) {
				ACT -= ACT_EXIT;
				return;
			}
	...
	scache(int port, InetAddress adr) {
		if (adr != null)
			...
			try {
				...
				System.err.println("[SMARTCACHE] Fatal error: Cannot bind to my port " + port + "/" + adr.getHostAddress() + "\n[SMARTCACHE]    Reason: " + e);
				System.exit(3);
			}
		else
			try {
				...
				System.err.println("[SMARTCACHE] Fatal error: Cannot bind to my port " + port + "/*" + "\n[SMARTCACHE]    Reason: " + e);
				System.exit(3);
			}
	}
