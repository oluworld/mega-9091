...
public final class httpreq implements Runnable {
        ...
        try {
            ...
            while (true) {
                ...
                if (req.startsWith("GET ")) {
                    req_method = REQUEST_GET;
                } else if (req.startsWith("POST ")) {
                    req_method = REQUEST_POST;
                } else if (req.startsWith("HEAD ")) {
                    req_method = REQUEST_HEAD;
                } else if (req.startsWith("OPTIONS ")) {
                    req_method = REQUEST_OPTIONS;
                } else if (req.startsWith("PUT ")) {
                    req_method = REQUEST_PUT;
                } else if (req.startsWith("TRACE ")) {
                    req_method = REQUEST_TRACE;
                } else if (req.startsWith("DELETE ")) {
                    req_method = REQUEST_DELETE;
                } else if (req.startsWith("CONNECT ")) {
                    req_method = REQUEST_CONNECT;
                } else if (req.startsWith("PROPFIND ")) {
                    req_method = REQUEST_PROPFIND;
                } else if (req.startsWith("REPORT ")) {
                    req_method = REQUEST_REPORT;
                } else if (req.startsWith("MOVE ")) {
                    req_method = REQUEST_MOVE;
                } else if (req.startsWith("COPY ")) {
                    req_method = REQUEST_COPY;
                } else if (req.startsWith("MKCOL ")) {
                    req_method = REQUEST_MKCOL;
                } else if (req.startsWith("PROPPATCH ")) {
                    req_method = REQUEST_PROPPATCH;
                } else if (req.startsWith("MKACTIVITY ")) {
                    req_method = REQUEST_MKACTIVITY;
                } else if (req.startsWith("CHECKOUT ")) {
                    req_method = REQUEST_CHECKOUT;
                } else if (req.startsWith("MERGE ")) {
                    req_method = REQUEST_MERGE;
                } else {
                    server_error(httpv, 501, "Unknown request method", ou);
                }

                /* pro HTTP 1.0+ - musime precist zbytek volovin az do prazdne radky */
                if (httpv > 9) {
                    while (true) {
                        req = in.readLine();
                        if (req == null) {
 ...
 /* zaciname zpracovavat requesty */
        ...
        } /* main loop try */ catch (IOException main_transfer_error) {
        } finally {
    ...
    public static boolean
        ...
        if (xlogfilez[i] != null) {
            return true; // allready open
        }
        if (xlogfilenames[i] == null) {
            return false; // unknown name
        }
        /* OPEN LOGFILE */
        try {
            DataOutputStream dos;
            dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(xlogfilenames[i], true), 8192));
            xlogfilez[i] = dos;
            ...
            for (int i = 0; i < j; i++) {
                ...
                if (outfile == null) {
                    if (logfilenames[i] != null)
                        ...
                        /* OPEN LOGFILE */
                        if (!open_logfile(logfilenames, logfilez, i)) {
                            break;
                        } else {
                            outfile = logfilez[i];
                        }
