 ...
 *  Copyright (C) 1999-2003 Radim Kolar
 *
...
public class repair {
    ...
    public static boolean deamp = false;

    public static String defaultype = "text/html";
    ...
    public static void main(String argv[]) {
        ...
        boolean rec;
        rec = false;
        if (cacheobject.defaultname == null) {
            cacheobject.defaultname = mgr.DEFAULTNAME;
        }
        ...
        for (int i = 0; i < argv.length; i++) {
            ...
            if (argv[i].equals("-x")) {
                if (i < argv.length - 1) {
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        ignoretable = mgr.addRegexpToArray(token, ignoretable, true);
                        i++;
                        continue;
                    }
                } else {
            ...
            if (argv[i].equals("-i")) {
                if (i < argv.length - 1) {
                    cacheobject.defaultname = argv[i + 1];
                    i++;
                    continue;
                } else {
                    continue;
        ...
        if (anydir) {
            System.out.println("[OK] Rebuild done.\n");
        } else {
            usage();
        }
    }
    ...
    public static cachedir repairDir(String dirname, boolean recurse) {
        ...
        for (int i = filez.size() - 1; i >= 0; i--) {
            ...
            if (!df.isDirectory()) {
                ...
                if (!quiet) {
                    System.out.println("[INFO] Adding orphan file: " + df + " ct=" + ctype);
                }
                cacheobject co;
                if (nm.equals(cacheobject.defaultname)) {
                    nm = "";
                }
    ...
    public static void deampDir(cachedir cd) {
        ...
        while (en.hasMoreElements()) {
            ...
            if (o2.getLocalName() != null) {
                /* zjistit, ktery je pozdejsi */
                if (o2.getDate() > o1.getDate()) {
                    System.out.println("[INFO] Removing " + cd.getLocalDir() + n1 + " URL name same as deamp version, but older.");
                    if (!nosave) {
                        o1.delete();
                    }
                    continue;
                } else {
                    System.out.println("[INFO] Removing older " + cd.getLocalDir() + n1 + " Making name available for newer version.");
                    if (!nosave) {
                        o2.delete();
                    } else {
                        cd.remove(o2);
            ...
            /* rename o1 to n2 */
            o1.setName(n2);
            System.out.println("[INFO] Unquoting " + cd.getLocalDir() + n1 + " as " + n2);
        }
    }
    ...
    public static String guessContentType(String fname) {
        ...
        for (int i = 0; i < guesstable.length; i += 2) {
            if (fname.endsWith(guesstable[i])) {
    ...
    private static void updateGuessTable(String mimetype, String ext) {
        ...
        if (mimetype.length() == 0 || ext.length() == 0) {
            return;
        }
        ext = ("." + ext).toLowerCase();
        for (int i = 0; i < guesstable.length; i += 2) {
    ...
    private static void usage() {
        ...
        System.out.println("  -c            compress files with gzip");
        ...
        System.out.println("  -m <file>     use alternate table for determining mime type (! to reset)");
        System.out.println("  -d <mime type>content type for unknown files (default text/html)");
        System.out.println("  -a            convert &amp; to & in URL");
        System.exit(1);
        return;
    }
}
