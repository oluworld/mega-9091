		...
		if (cd != null) {
			...
			if (i != -1) {
				...
				if (i != -1)
					...
					if (gcloglevel > 3) System.out.println(new Date() + " Scanning " + dirname);
					urlpart = "";
				}
			} else { urlpart = "";}
			...
			while (en.hasMoreElements()) {
				...
				if (obj.getRC() >= 400)
					...
					/* redirect ? */ if (obj.isRedirect())
						w = applyMod(w, redirpenal);
					else if (chk == false) w = applyMod(w, notchpenal);
				}
...
/* vraci true pokud modifikator nic nedela */
	...
	private cachedir kill_unref_dir(String dirname, boolean recurse) {
		...
		if (!lroot.isDirectory()) return null;
		dirfilez = lroot.list();
