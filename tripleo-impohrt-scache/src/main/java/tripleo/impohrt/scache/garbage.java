...
public final class garbage {
    ...
    public static final String GCVERSION = "0.24";
    public static final float REFAGE_MAX = 999999;
    /* nic se nemaze pokud je >= */
 ...
 /* 5 - debug info */
    ...
    private int dirsize;
    /* average dirsize */
    private long oursize;
    /* current cache size */
    private int ourdirs;
    /* directory count in cache */
    private int ourfiles;
    /* files count in cache */
    public int scandel;
    /* files deleted during killunref or GC scan */

    private int lowmark, highmark;
    private int maxsize, minsize;
    private float refage;
    /* marks for fast file deletion */
    ...
    private void optimise() {
        ...
        if (lopact1 == null) {
            lopsize = Integer.MAX_VALUE;
        }
        if (lopact2 == null) {
            lopsizeinc = Integer.MAX_VALUE;
        }
        ...
        if (dummyMod(exppenal)) {
            exppenal = null;
        }
        if (dummyMod(notchpenal)) {
            notchpenal = null;
        }
        if (dummyMod(ebc)) {
            ebc = null;
    ...
    private void read_config(String cfgfile) {
        try {
            ...
            StringTokenizer st;
            int lineno = 0;
            if (!new File(cfgfile).isAbsolute()) {
            ...
            while ((line = dis.readLine()) != null) {
                ...
                try {
                    ...
                    if (token.equalsIgnoreCase("cache_swap_size")) {
                        cachesize = sizestring(st.nextToken("\r\n"));
                        continue;
                    }

                    if (token.equalsIgnoreCase("average_directory_size")) {
                        dirsize = (int) sizestring(st.nextToken("\r\n"));
                        continue;
                    }

                    if (token.equalsIgnoreCase("cache_swap_low")) {
                        lowmark = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }

                    if (token.equalsIgnoreCase("cache_swap_high")) {
                        highmark = Integer.valueOf(st.nextToken()).intValue();
                        continue;
                    }
                    if (token.equalsIgnoreCase("block_size")) {
                        blocksize = (int) sizestring(st.nextToken("\r\n"));
                        continue;
                    }

                    if (token.equalsIgnoreCase("gcarraysize")) {
                        gcarray = new cacheobject[Integer.valueOf(st.nextToken()).intValue()];
                    ...
                    if (token.equalsIgnoreCase("urlmask")) {
                        token = st.nextToken();
                        if (mgr.isInRegexpArray(token, urlmask) > -1) {
                            continue;
                        }
    ...
    private void import0(String improot, String dirname) {
        ...
        for (int i = filez.length - 1; i >= 0; i--) {
            if (new File(dirname + File.separator + filez[i]).isDirectory()) {
                import0(improot, dirname + File.separator + filez[i]);
            }
        }
 ...
 /* root=d:\store
        ...
        for (String s : dirfilez) {
            if (!new File(myroot, s).isDirectory()) {
                continue;
            }
            if (goodlevel == true) {
                // extract hostname from filename
 ...
 /* *://www.debian.org/* -> www.debian.org(*)/     */
    ...
    public final void garbage_collection() {
        ...
        while (oursize > high) {
            ...
            scan_cache(root);
            if (scandel > 0 && gcloglevel > 0) {
                System.out.println(new Date() + " " + scandel + " files deleted during scan.");
            }
            if (gcloglevel > 1) {
                print_gc_outro();
            }
    ...
    public final void fake_garbage_collection() {
        ...
        scan_cache(root);
        if (scandel > 0) {
            System.out.println(new Date() + " " + scandel + " bad files deleted during scan.");
        }
        print_gc_outro();
        dump_gcarray();
        fake_delete_files();
    ...
    private void delete_files() {
        ...
        if (oursize < bot) {
            if (gcloglevel > 0) {
                System.out.println(new Date() + " Cache size is bellow lowmark - No cleaning needed.");
            }
            return;
        }
        ...
        for (int i = gcused; i >= 0; i--) {
            ...
            if (gcloglevel > 3) {
                System.out.println("Deleted: " + gcarray[i].getDirectory().getLocalDir() + gcarray[i].getLocalName() + " age=" + gcvalues[i]);
            }
            gcarray[i].delete();
            int size;
            ...
            if (oursize < bot) {
                if (gcloglevel > 1) {
                    System.out.println(new Date() + " Low watermark reached.");
                }
                break;
            }
        }
        if (gcloglevel > 0) {
            System.out.println(new Date() + " Deleted " + files + " files from cache.");
        }
    ...
    private void fake_delete_files() {
        ...
        if (oursize > high) {
            System.out.println(new Date() + " [FAKE] WARNING: Another rescan cache will be needed to finishing real GC.");
        }
    }
 ...
 /* udelat name */
            ...
            if (i != -1) {
                ...
                if (i != -1) {
                    ...
                    if (gcloglevel > 3) {
                        System.out.println(new Date() + " Scanning " + dirname);
                    }
                    urlpart = "";
                }
            } else {
            ...
            while (en.hasMoreElements()) {
                ...
                if (size > maxsize) {
                    if (gcloglevel > 3) {
                        System.out.println("Deleted too large object: " + dirname + File.separator + obj.getLocalName() + " size=" + size + " B, maxsize=" + maxsize + " B.");
                    }
                    obj.delete();
                    scandel++;
                    continue;
                    ...
                    if (gcloglevel > 3) {
                        System.out.println("Deleted too small object: " + dirname + File.separator + obj.getLocalName() + " size=" + size + " B, minsize=" + minsize + " B.");
                    }
                    obj.delete();
                    scandel++;
                    continue;
                ...
                if (obj.getRC() >= 400) {
                    ...
                    } else /* redirect ? */ if (obj.isRedirect()) {
                        w = applyMod(w, redirpenal);
                    } else if (chk == false) {
                        w = applyMod(w, notchpenal);
                    }
                }
    ...
    private cachedir kill_unref_dir(String dirname, boolean recurse) {
        ...
        String dirfilez[];
        String local[];
        Vector<String> filez = new Vector<String>();
        ...
        dirfilez = lroot.list();
        if (dirfilez == null) {
            return null; // WAS: dirfilez=new String[0]; WHY?
        }
        cachedir cd = new cachedir(dirname + File.separator, null);
        /* VERIFY PART #1: Check files in .cacheinfo */
        if (!cd.checkDir()) {
            cd.save();
        }
        /* VERIFY PART #2: Remove files not found in .cacheinfo */
        ...
        /* VERIFY PART #3: try to delete empty dir */
        if (dirisempty) {
            // System.out.println("/debug/ killunref Cleanimg ed: "+dirname);
            cd.cleandir();
            rc = null;
        }

        /* VERIFY PART #4: Recurse into subdirs */
        if (recurse) {
            cd = null;
            filez.trimToSize();
            for (int i = filez.size() - 1; i >= 0; i--) {
    ...
    private void importcache0(String dirname) {
        ...
        for (String value : dirfilez) {
            if (!new File(myroot, value).isDirectory()) {
                continue;
            }
            try {
            ...
            for (String s : subdirz) {
                if (!new File(new File(myroot, value), s).isDirectory()) {
                    continue;
                }
                try {
                    Integer.valueOf(s);
 ...
 /* export */
    private void export_one_dir(String dirname, String to, int type, long difftime) {
        ...
        if (type != EXPORT_ALL) {
            now = System.currentTimeMillis();
            Enumeration<cacheobject> en = cd.getObjects();
            while (en.hasMoreElements()) {
                cacheobject obj;
                obj = en.nextElement();
                if (type == EXPORT_LRU) {
                    if (now - obj.getLRU() <= difftime) {
                        export = true;
                        break;
                    } else {
                        continue;
        ...
        for (int i = filez.length - 1; i >= 0; i--) {
            if (new File(dirname + File.separator + filez[i]).isDirectory()) {
                export_one_dir(dirname + File.separator + filez[i], to, type, difftime);
            }
        }
    }
