 ...
 *     }
 *   }
 * </pre> Instead, if the values are intended to be the same in each case, just
 * take advantage of the fact that put returns null if the key was not
 * previously present:
 * <pre>
...
public class ConcurrentReaderHashMap
    ...
    * Remove() (also clear()) invalidates removed nodes to alert read
     ...
     * elements is less than zero, or if the load factor is nonpositive.
     */
     ...
     * <tt>ConcurrentReaderHashMap</tt> instance: the keys and values themselves
     * are not cloned.
     *
    ...
    protected static class Entry implements Map.Entry {
       ...
       marked as final. 
         ...
         * <tt>getValue</tt> <em>might</em>
         * return null, reflecting the fact that the entry has been concurrently
         * removed. However, there are no assurances that concurrent removals
         * will be reflected using this method.
         *
         ...
         * obtained via the <tt>get</tt> operation of the underlying hash table
         * in multithreaded applications. If iterator-wide synchronization is
         * not used, and any other concurrent
         * <tt>put</tt> or <tt>remove</tt> operations occur, sometimes even to
         * <em>other</em> entries, then this change is not guaranteed to be
         * reflected in the hash table. (It might, or it might not. There are no
     ...
     * mappings), followed by the key (Object) and value (Object) for each
     * key-value mapping represented by the ConcurrentReaderHashMap The
