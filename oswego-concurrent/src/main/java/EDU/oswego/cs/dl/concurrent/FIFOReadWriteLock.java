...
/**
 * This class implements a policy for reader/writer locks in which threads
 * contend in a First-in/First-out manner for access (modulo the limitations of
 * FIFOSemaphore, which is used for queuing). This policy does not particularly
 * favor readers or writers. As a byproduct of the FIFO policy, the
 * <tt>attempt</tt> methods may return <tt>false</tt> even when the lock might
 * logically be available, but, due to contention, cannot be accessed within the
 * given time bound.
 * <p>
 *
 * This lock is <em>NOT</em> reentrant. Current readers and writers should not
 * try to re-obtain locks while holding them.
 * <p>
 *
...
public class FIFOReadWriteLock implements ReadWriteLock {
    ...
    /**
     * Fair Semaphore serving as a kind of mutual exclusion lock. Writers
     * acquire on entry, and hold until rwlock exit. Readers acquire and release
     * only during entry (but are blocked from doing so if there is an active
     * writer).
     *
     */
    ...
    /**
     * Number of threads that have exited read lock. Note that this is never
     * reset to zero. Accessed only in code protected by synchronized(this).
     * When exreaders != readers, the rwlock is being used for reading. Else if
     * the entry lock is held, it is being used for writing (or in transition).
     * Else it is free. Note: To distinguish these states, we assume that fewer
     * than 2^32 reader threads can simultaneously execute.
     *
     */
