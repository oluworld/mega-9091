 ...
 * Elements are serialized using ObjectInputStreams and ObjectOutputStreams upon
 * insertion and extraction from the pipe.
 * <p>
 * IO Exceptions are transformed into Errors. This is in general not a good
 * idea, but seems to be the most reasonable compromise for the intended usage
 * contexts.
 * <p>
 * <b>Status</b> Uncertain. There are enough conceptual and implementation snags
 * surrounding use of pipes as Channels to downplay use. However, without such
 * bridges, people would have to duplicate code that should work the same way in
 * both cases.
 *
 * <p>
...
public class PipedChannel extends SemaphoreControlledChannel {
    ...
    /**
     * Return input stream, first constructing if necessary. Needed because
     * Object streams can block on open.
     *
     */
    protected synchronized ObjectInputStream in() {
        try {
            if (in_ == null) {
                in_ = new ObjectInputStream(inp_);
            }
            return in_;
        } catch (IOException ex) {
            ex.printStackTrace();
    ...
    /**
     * Return output stream, first constructing if necessary. Needed because
     * Object streams can block on open.
     *
     */
    protected synchronized ObjectOutputStream out() {
        try {
            if (out_ == null) {
                out_ = new ObjectOutputStream(outp_);
            }
            return out_;
        } catch (IOException ex) {
            ex.printStackTrace();
