 ...
 * <
 * pre>
 * class ImageRenderer { Image render(byte[] raw); } class App { Executor
 * executor = ... ImageRenderer renderer = ... void display(byte[] rawimage) {
 * try { FutureResult futureImage = new FutureResult(); Runnable command =
 * futureImage.setter(new Callable() { public Object call() { return
 * renderer.render(rawImage); } }); executor.execute(command); drawBorders(); //
 * do other things while executing drawCaption();
 * drawImage((Image)(futureImage.get())); // use future } catch
 * (InterruptedException ex) { return; } catch (InvocationTargetException ex) {
 * cleanup(); return; } } }
 * </pre>
 * <p>
