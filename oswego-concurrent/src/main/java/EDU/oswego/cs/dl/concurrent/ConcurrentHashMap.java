 ...
 * are designed to be used by only one thread at a time. Passing an iterator
 ...
 * <dt> Updates
 *
 * <dd> This class supports a hard-wired preset <em>concurrency level</em> of
...
public class ConcurrentHashMap
    ...
    * Locking for puts, removes (and, when necessary gets, etc)
     ...
     * This value is used in an approximate way: When at least a quarter of the
     * segments of the table reach per-segment threshold, or one of the segments
     * itself exceeds overall threshold, the table is doubled. This will on
     * average cause resizing when the table-wide load factor is slightly less
     * than the threshold. If you'd like to avoid resizing, you can set this to
     * a ridiculously large value.
     ...
     * <code>null</code> if the key is not mapped to any value in this table.
     ...
     * this table, as determined by the
     ...
     * @param assumedTab the state of table on first call to resize. If this
     * changes on any call, the attempt is aborted because the table has already
     ...
     * <tt>ConcurrentHashMap</tt> instance: the keys and values themselves are
     * not cloned.
     *
    ...
    private class EntrySet extends AbstractSet {
        ...
        public boolean remove(Object o) {
            ...
            return ConcurrentHashMap.this.remove(e.getKey(), e.getValue()) != false;
        }
    ...
    protected static class Entry implements Map.Entry {
       ...
       marked as final. 
         ...
         * concurrent <tt>put</tt> or <tt>remove</tt>
         * operations occur, sometimes even to <em>other</em> entries, then this
         * change is not guaranteed to be reflected in the hash table. (It
