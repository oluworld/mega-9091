...
/**
 * Microscope implements a version of the 7th Guest game found looking in the
 * Microscope in the laboratory. See
 * <a href="http://gee.cs.oswego.edu/dl/applets/micro.html">
 * Microscope</a> version for instructions.
 * <p>
 * The code has been mangled beyond recognition as a test of the FJTasks
 * package.
 *
 */
public class Microscope extends JPanel {
   ...
   * This is useful for testing purposes, but much less fun to watch.
    ...
    /**
     * Board configurations are represented by bit vectors. Since there are only
     * 49 cells, the bits can be held in `longs', one for each player.
     * <p>
     * Boards are not immutable, but are never passed around across threads
     * (instead new ones are constructed), so don't need any synch.
     *
     */
    ...
    static final class Move {
        ...
        synchronized boolean isJump() {
            return (fromRow - toRow == 2) || (toRow - fromRow == 2)
        ...
        synchronized void commit() { // update board to reflect move
            if (!committed) {
                ...
                if (isLegal() && !isPass()) {
                    if (isJump()) {
                        board_.occupy(Player.Empty, fromRow, fromCol);
                    }
    ...
    /**
     * Mover is an abstract class to simplify code dealing with either user
     * moves or auto moves.
     *
     */
    ...
    /**
     * Implements a classic all-possible-move search algorith using FJTasks. The
     * move finder is not all that smart. Among other possible improvements, it
     * could keep a cache of explored moves and avoid repeating them. This would
     * likely speed it up since most expansions are duplicates of others. It
     * could also be changed to prune moves, although this is unlikely to work
    ...
    static class RootFinder extends Finder {
        ...
        /**
         * This differs from default version by recording and calling back with
         * best move
         *
         */
        void collect(Finder forked) {
