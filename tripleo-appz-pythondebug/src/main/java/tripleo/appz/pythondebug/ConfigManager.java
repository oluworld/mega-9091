 ...
 *   $Id: ConfigManager.java,v 1.1.1.1 2005/06/20 08:00:35 olu Exp $
 *
...
//		strokes.put(new Float(1.0f), new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, SVGWriter.DEFAULT_MITER_LIMIT));  //not using default as default values for some props are different from SVG
    ...
    void initWindows() {
        ...
        tabY = mainH;
        tabW = screen.width;
        tabH = screen.height - mainH;
        application.propsp = new PropsPanel(application, prpX, prpY, prpW, prpH);
        if (Utils.osIsWindows()) {
            tabH -= 28;
        } //if we have a Windows GUI, take the taskbar into account
...
//	application.tblp.setSize(tabW,tabH);
        ...
        if (!showEditWindow) {
            ...
            application.propsp.setVisible(true);
        }
        if (!showNSWindow) {
            application.cmp.showTablesMn.setSelected(false);
        }
//	else {application.tblp.setVisible(true);}
        if (!showNavWindow) {
            application.cmp.showNavMn.setSelected(false);
        }
...
//	Editor.mView.setVisible(true);
    ...
    void updateWindowVariables() {
        cmpX = application.cmp.getX();
        cmpY = application.cmp.getY();
        cmpW = application.cmp.getWidth();
        cmpH = application.cmp.getHeight();
...
//	mainW=Editor.mView.getFrame().getWidth();mainH=Editor.mView.getFrame().getHeight();
        prpX = application.propsp.getX();
        prpY = application.propsp.getY();
        prpW = application.propsp.getWidth();
        prpH = application.propsp.getHeight();
