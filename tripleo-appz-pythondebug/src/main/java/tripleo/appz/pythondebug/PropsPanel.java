 ...
 *   $Id: PropsPanel.java,v 1.1.1.1 2005/07/14 03:26:20 olu Exp $
 *
...
class PropsPanel extends JFrame {
	...
	PropsPanel(PythonDebug e, int x, int y, int width, int height) {
		this.application = e;
		...
		WindowListener w0 = new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				application.cmp.showPropsMn.setSelected(false);
			}

			public void windowActivated(WindowEvent e) {
				application.alwaysUpdateViews(true);
			}
		};
...
//		if (o instanceof IResource) {this.showResourceProps((IResource) o);} else if (o instanceof ILiteral) {
//			this.showLiteralProps((ILiteral) o);
				...
				void checkAndUpdateResource() {
					if (tc.getText().length() > 0) {
						if (!tc.getText().startsWith(Editor.ANON_NODE)) {
							...
							javax.swing.JOptionPane.showMessageDialog(application.propsp, Editor.ANON_NODE + " is reserved for anonymous nodes.");
						}
					} else {
						javax.swing.JOptionPane.showMessageDialog(application.propsp, Messages.provideURI);
					}
				}
			};
			...
			ActionListener a2 = new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (e.getSource() == delete) {
						...
						RDFDatatype dt = Editor.displayAvailableDataTypes(tfType.getText());
						if (dt != null) {
							l.setDatatype(dt);
...
class Messages {
	...
	protected static final Object provideURI = null;}
