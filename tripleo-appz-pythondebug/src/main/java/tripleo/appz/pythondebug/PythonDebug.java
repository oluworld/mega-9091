 ...
 *   $Id: PythonDebug.java,v 1.1.1.1 2005/07/14 22:09:23 olu Exp $
 *
...
/*		if (m_TmpDir.exists()) {
			if (gssFile != null) {
				...
				if (f2.exists()) {
					if (argFile.endsWith(".nt")) {
						gssMngr.loadStylesheet(f2, RDFLoader.NTRIPLE_READER);
					}  //1 is NTriples reader
					else if (argFile.endsWith(".n3")) {
						gssMngr.loadStylesheet(f2, RDFLoader.N3_READER);
					}  //2 is Notation3 reader
			...
			if (argFile != null) {//load/import file passed as command line argument
				if (argFile.endsWith(".isv")) {isvMngr.openProject(new File(argFile));} else if (argFile.endsWith(".nt")) {
					loadRDF(new File(argFile), RDFLoader.NTRIPLE_READER, true);
				}  //1 is NTriples reader
				else if (argFile.endsWith(".n3")) {
					loadRDF(new File(argFile), RDFLoader.N3_READER, true);
				}  //2 is Notation3 reader
...
//						rdfLdr.loadAndStyle(f, whichReader);
...
//						Editor.mView.setCursorIcon(java.awt.Cursor.WAIT_CURSOR);
//						if (gssMngr.getStylesheetList().size() > 0) {
//							rdfLdr.loadAndStyle(u, whichReader);
//						}//if a least one stylesheet is declared,
...
//					Editor.mView.setCursorIcon(java.awt.Cursor.WAIT_CURSOR);
//					if (gssMngr.getStylesheetList().size() > 0) {
//						rdfLdr.loadAndStyle(is, whichReader);
//					}//if a least one stylesheet is declared,
...
//						errorMessages.append("Editor.mergeRDF() " + ex + "\n");
//						reportError = true;
//					}
...
//					if (gssMngr.getStylesheetList().size() > 0) {
//						rdfLdr.loadAndStyle(tmpF, RDFLoader.RDF_XML_READER);
...
//					generateJenaModel();
//					//load second model only as a Jena model and merge it with first one
//					try {rdfModel.add(rdfLdr.merge(u, whichReader));} catch (RDFException ex) {
//						errorMessages.append("Editor.mergeRDF() " + ex + "\n");
//						reportError = true;
//					}
//					//serialize this model in a temporary file (RDF/XML)
...
//				if (b) {vs.show(r.getGlyphText());} else {vs.hide(r.getGlyphText());}
//			}
//		}
...
//		if (el.getWidth() < (1.5 * el.getHeight())) {
//			el.setWidth(Math.round(1.5 * el.getHeight()));
//		}
//		//center VText in ellipse
//		g.moveTo(el.vx - (long) r2d.getWidth() / 2, el.vy - (long) r2d.getHeight() / 4);
//		if (r.isAnon() && !ConfigManager.SHOW_ANON_ID) {
//			vsm.getVirtualSpace(mainVirtualSpace).hide(g);
//		}
...
//			vsm.getVirtualSpace(mainVirtualSpace).hide(r.getGlyphText());
//		}
//		gssMngr.incStyling(r);
//	}
//
...
//						r.setAnon(false);
//						if (!ConfigManager.SHOW_ANON_ID) {
//							vsm.getVirtualSpace(mainVirtualSpace).show(r.getGlyphText());
//						}
//					}
//					r.setURI(uri);
//					r.setURIFragment(false);
//					resourcesByURI.put(r.getIdentity(), r);
//				} else {
//					JOptionPane.showMessageDialog(propsp, "A resource with URI " + uri + " already exists");
//				}
//			}
//		} else {//a local ID
...
//					resourcesByURI.put(r.getIdentity(), r);
//				} else {
//					JOptionPane.showMessageDialog(propsp, "A resource with ID " + uri + " already exists");
//				}
//			}
//		}
//		String qname = r.getGraphLabel();
...
//		if (resourcesByURI.containsKey(uri)) {return true;} else {return false;}
//	}
//
//	/*when the user creates a new resource from scratch in the environment*/
...
//			if (l.getGlyphText() != null) {l.getGlyphText().setText(truncText);} else {//literal is empty, so it does not have an associated VText - have to create it
//				Glyph rt = l.getGlyph();
...
//			if (DISP_AS_LABEL) {
//				geomMngr.adjustResourceTextAndShape(p.subject, p.subject.getLabel());
//			}
//		}
...
//		boolean error = false;
//		if ((object instanceof ILiteral) && (((ILiteral) object).getIncomingPredicate() != null)) {
//			error = true;
...
//			IResource oldSubject = p.getSubject();
//			oldSubject.removeOutgoingPredicate(p);
//		}
...
//			if (oldObject instanceof ILiteral) {
//				((ILiteral) oldObject).setIncomingPredicate(null);
...
//				if (key.indexOf(uriFragment) != -1) {
//					selectResource((IResource) resourcesByURI.get(key), true);
//				}
//			}
...
//							if ((p.getNamespace().indexOf(trueNS) != -1) && (p.getLocalname().indexOf(lnFragment) != -1)) {
...
//			l.setSelected(b);
//			if (b) {
//				if (!selectedLiterals.contains(l)) {
//					selectedLiterals.add(l);
//					lastSelectedItem = l;
//				}
...
//								if (select) {
//									if (!selectedPredicates.contains(tmpP)) {selectedPredicates.add(tmpP);}
//								} else {selectedPredicates.remove(tmpP);}
//							}
//						}
//					} catch (NullPointerException ex) {
//						System.err.println("Error:Editor.selectPredicate(): unable to select property " + p);
//						ex.printStackTrace();
//					}
//				}
//			} else {
...
//				if (select) {
//					if (!selectedPredicates.contains(p)) {
//						selectedPredicates.add(p);
//						lastSelectedItem = p;
//					}
...
//		INode n = p.getObject();
//		if (n instanceof IResource) {selectResource((IResource) n, true);} else {
//			selectLiteral((ILiteral) n, true);
//		}//instanceof ILiteral
//	}
//
...
//			System.err.println("Debug: Editor.unselectAll(): selectedLiterals should be empty ; size=" + selectedLiterals.size());
//		}
...
//				deleteProperty((IProperty) v.elementAt(i));
//			}
//		}
//		if ((v = r.getOutgoingPredicates()) != null) {
//			for (int i = v.size() - 1; i >= 0; i--) {
...
//			if (!finalBindings.containsKey(namespace)) {
//				namespacesWithoutAPrefix.add(namespace);
//			}
//		}
...
//				if (!silent) {
//					JOptionPane.showMessageDialog(tblp, "Prefix " + prefix + " is already assigned to namespace " + (String) tblp.nsTableModel.getValueAt(i, 1));
//				}
...
//				if (!silent) {
//					JOptionPane.showMessageDialog(tblp, "Namespace URI " + uri + " is already binded to prefix " + (String) tblp.nsTableModel.getValueAt(i, 0));
//				}
//				if (!override) {return false;} else {//conflict detected - override existing prefix and display_as_prefix in the table with new param values
...
//			prefAlreadyInUse = true;
//			if (!silent) {
//				JOptionPane.showMessageDialog(tblp, "Prefix '" + prefix + "' is already used as the anonymous node prefix");
//			}
...
//		if (tblp.nsTable.getRowCount() > n) {tblp.nsTable.setRowSelectionInterval(n, n);} else if (tblp.nsTable.getRowCount() > 0) {
...
//					JOptionPane.showMessageDialog(tblp, "Prefix " + ns + " is not binded to any namespace.");
//					return false;
//				}
//			} else {
//				namespace = ns;
...
//					if (!silent) {
//						JOptionPane.showMessageDialog(tblp, "Property " + namespace + ln + " is already defined.");
//					}
//					return false;
//				}
//			}
...
//				data.add(namespace);
//				data.add(prefix == null ? "" : prefix);
//				data.add(ln);//null as 2nd element
...
//						if (!(((String) tm.getValueAt(j, 0)).equals(RDFMS_NAMESPACE_URI) && Utils.isMembershipProperty(aLN))) {
//							lastMembershipPropIndex = j + 1;
//							break;
//						}
//					}
...
//						if (aProperty.compareTo(namespace + ln) > 0 || Utils.isMembershipProperty((String) tm.getValueAt(i, 2))) {
//							break;
//						}
//					}
//				}
...
//		for (Enumeration e = propertiesByURI.keys(); e.hasMoreElements();) {
//			if (((String) e.nextElement()).equals(uri)) {
//				used = true;
//				break;
//			}
//		}
...
//			if (option == JOptionPane.OK_OPTION) {
//				tm.removeRow(n);
//				if (tblp.prTable.getRowCount() > n) {
//					tblp.prTable.setRowSelectionInterval(n, n);
...
//		} else {
//			tm.removeRow(n);
//			if (tblp.prTable.getRowCount() > n) {tblp.prTable.setRowSelectionInterval(n, n);} else if (tblp.prTable.getRowCount() > 0) {
//				int i = tblp.prTable.getRowCount() - 1;
//				tblp.prTable.setRowSelectionInterval(i, i);
//			}
//		}
...
//		if (ConfigManager.showNSWindow) {tblp.show();} else {tblp.hide();}
//	}
//
...
//		if (ConfigManager.showEditWindow) {propsp.show();} else {propsp.hide();}
//	}
//
...
//		if (ConfigManager.showNavWindow) {navp.show();} else {navp.hide();}
//	}
//
//	/*global view*/
//	void getGlobalView() {
...
//				for (int i = 0; i < v.size(); i++) {
//					if ((((Glyph) v.elementAt(i)).getText() != null) && ((((Glyph) v.elementAt(i)).getText()).toLowerCase().indexOf(lastSearchedString) != -1)) {
//						matchingList.add(v.elementAt(i));
//					}
//				}
//			}
...
//		} catch (Exception ex) {
//			System.err.println("Error: Editor.displayRawFile: " + ex);
//			ex.printStackTrace();
//		}
//	}
//
...
//			System.out.print("[");
//			for (int i = 0; i < copiedLiterals.size() - 1; i++) {
//				System.out.print(copiedLiterals.elementAt(i).toString() + ",");
//			}
//			System.out.println(copiedLiterals.lastElement().toString() + "]");
//		}
//		if (!copiedPredicates.isEmpty()) {
//			System.out.print("[");
//			for (int i = 0; i < copiedPredicates.size() - 1; i++) {
//				System.out.print(copiedPredicates.elementAt(i).toString() + ",");
//			}
