...
public class SplashWindow extends JWindow implements Runnable, MouseListener {
    ...
    /**
     * @param d display splash screen for at least d milliseconds (0 means
     * infinite - until user clicks on the screen) - 'at least' because it will
     * anyway wait until the JProgressBar goes to 100% (you can set it at 100%
     * from the beginning if you want)
     * @param ip image path (from exec directory)
     * @param toFront true will periodically bring it to front so that it always
     * stay on top (might behave strangely on some systems)
     */
    public SplashWindow(int d, String ip, boolean toFront, Frame owner) {
        ...
        msg.setFont(PythonDebug.tinyFont);
        ...
        txtInfo.setFont(PythonDebug.tinyFont);
    ...
    public void run() {
        ...
        while (duration > 0) {//this loop should last about -duration- ms
            if (periodBringToFront) {
                this.toFront();
            } //we have no way of telling that a window should always be drawn on top of the others, so we bring it to front regularly
            try {
                runView.sleep(50);
            } catch (Exception e) {
                e.printStackTrace();
