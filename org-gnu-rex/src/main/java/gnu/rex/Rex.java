...
/**
 * This is a key class of the package. It allows you to invoke the tow main
 * actions: expression parsing and matching it against a string. The regular
 * expressions supported are a subset of lex plus some extensions:
 * <P>
 * <
 * Pre>
 * x	the character x. "x"	an x, even if x is an operator. \x	an x, even if x is
 * an operator. [xy]	the character x or y. [x-z]	the character x, y or z. [^x]
 * any character but x. .	any character but newline. ^x	an x at the beginning of
 * a line. x$	an x at the end of a line. x?	an optional x. x*	0 or more
 * instances of x. x+	1 or more instances of x. x\|y	an x or a y. \(x\)	an x
 * matched as a subexpression. x{m,n}	m through n occurences of x.
 *
...
public class Rex {
     ...
     * @exception RegExprSyntaxException if regExpr has unacceptable syntax
     */
     ...
     * (see [^x-z] above ).
     * @param name a one character name for the class. Only upper case names are
     ...
     * @return the set of pairs each representing offset and length of the
     * corresponding subexpression. Only 9 (1 through 9) subexpressions are
     * supported. The pair at 0 element represents the entire matched region.
     */
