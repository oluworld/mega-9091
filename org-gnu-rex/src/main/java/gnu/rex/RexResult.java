...
public class RexResult {
  ...
  * and one overall result it requires 10 place-holders.
    ...
    RexResult() {
        this(DFL_MAX_RESULTS);
    }

    RexResult(int maxResults) {
        start = new int[maxResults];
        leng = new int[maxResults];
    }

    /**
     * returns the offset into the input stream of characters at which the
     * matched string begins.
     */
    ...
    /**
     * returns the offset into the input stream of characters at which the
     * matched substring begins. The offset is relative to the origin of the
     * entire string that has matched a given regular expression. In other
     * words, it's relative to the offset returned by offset() or offset(0).
     *
     * @param n sub-expression number
     */
    public final int offset(int n) {
        return n >= start.length ? -1 : start[n];
    }
