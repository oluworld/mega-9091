...
class Parser {
  ...
  *	END Of Config Stuff
    ...
    private void push(Expr exp) {
        exprStack.push((Object) exp);
        subExpr.addElement(exp);
    }
            ...
            switch (current) {
                ...
                case LT_0MAX:
                    if (lastAtom != null
                        ...
                        ++off;
                        lastMin = 0;
                        lastMax = Atom.MAX_VALUE;
                    } else if (!fallThrough) {
                        disabledSpecChar[SPEC_CHARS.indexOf(LT_0MAX)] = true;
                ...
                case LT_1MAX:
                    if (lastAtom != null
                        ...
                        disabledSpecChar[SPEC_CHARS.indexOf(LT_1MAX)] = true;
                    ...
                    if (validEndOnly(sub)) {
                        ...
                        if (inp.charAt(off) == LT_ESC) {
                            if (Character.isUpperCase(inp.charAt(off + 1))) {
                                Atom atom = CharClassVec[(int) inp.charAt(off + 1) - (int) 'A'];
    ...
    private int parse(String inp, int off) {
        ...
        for (exprStack = new Stack(); state != ST_DONE;) {
            ...
            switch (state) {
                ...
                case ST_A:
                    /* this state corresponds to an atom */
                    switch (lastToken) {
                        case TOK_N:
