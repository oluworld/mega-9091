...
class StateMachine {
  ...
  * MATCH Methods Section
    ...
    private int match(int offset, int lim, short startSt) {
        ...
        for (; offset <= lim; offset++) // original end string comparement for( ; offset <= lim; offset++);
        {
            c = input[offset];
            ...
            if (pCount == 0) // there's no transition to any state on c
            {
                break;
            }
  ...
  * Anchors Handling
    ...
    private boolean isEol(int off, int lim) {
        System.out.println("EOL test, off=" + off);
        pastEol = off + 1 >= lim || input[off] == EOL_CHAR;
        if (pastEol) {
            System.out.println("EOL,off=" + off);
        }
        return pastEol;
    }
