...
%constant void (*errCallback)(char *msg,int errcode) = NULL;

//%inline
//void DefaultErrorCallback(char *err_msg,int err_code) {
...
//~ int clServerByService(const char* service,
	 //~ int backlog, void (*threadFun)(struct clThreadData *),struct clThreadData* data) {
    //~ return clServerByPort(clGetPortByService(service),backlog,tfun,data);
//~ }
//~ }
...
typedef char byte ;
	...
	extern void bind(char*addr, int addr2);
	extern void close();
	extern Err connect_ex(char *address);
	extern int fileno();
	extern int getsockopt(int layer, int option);
	extern void listen(int num);
	extern byte[] recv(int buffer_size);
	extern int send(byte[] data);
	extern void setblocking(boolean f);
	extern void setsockopt(int layer, int option, int value);
