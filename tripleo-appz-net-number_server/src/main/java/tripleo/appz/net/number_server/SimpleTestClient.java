...
//	private volatile boolean cont = true;
	...
	/**
	 * after this is done, we can call init again or reinit
	 * and then call #doWork
	 *
	 */
	public void doWork() {
		_init();
...
//		p = new STC_Parent(useProxy, p.space());
	...
	class DD {
		 ...
		 * dd = DD()
		 * dd.pre()
		 * [ xx1.onSubmit(MYFJTask(p, STC_Request(iter)))
	...
	public List<STC_Response> responses() {
		...
		//
		if (p != null && p.responses != null) R = p.responses;
...
//		V.add(new Long(tt.t));
	...
	/**
	 * send an http req over the channel, and save the response to
	 * our resp vector
	 *
	 * @param aRequest
	 */
