...
class World implements Runnable {
	...
	public void run() {
		...
		synchronized (this) {
			for (int i = 0; i < 5; i++) {new Ghost(this, ghost_pos, player);}
		}
	...
	/**
	 * Check's if it's possible to move in the specified direction from the xy pos
	 */
	...
	public boolean containPowerup(int x, int y) {
		...
		if (level.powerupAt(pos)) {
			...
			if (player.getKillable()) Sound.sirenSound.play(Const.POWERUPTIME);
			return true;
		} else
			return false;
	}
