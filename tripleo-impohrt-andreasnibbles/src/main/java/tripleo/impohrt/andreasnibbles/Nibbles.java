...
public class Nibbles extends JPanel /*Applet*/ implements Runnable {
	...
	int dir = Worm.RIGHT;  	// Riktningen f�r masken (1,2,3,4)=(D,U,R,L).
	...
	public void run() {
		while (true) {	// For�vv�r, m��n.
			...
			while (!worm_CPU.move(dir_CPU)) {
				dir_CPU = (int) ((Math.random() * 4) + 1);
				koll++;
				if (koll > 10) {  // Om den inte hittat en utv�g efter 10 gissningar
					a1(worm, scoreplayer, 2000);
//					grid.clear(); // s� antar jag att det inte finns n�gon.
//					worm.score++;
...
//					try { Thread.sleep(2000); } catch (InterruptedException e) { }
	...
	private void a1(final Worm aWorm, final JLabel aScoreLabel, final int delay) {
		grid.clear();
		aWorm.score++;		// �ka datorns po�ng med 1.
		aScoreLabel.setText(Integer.toString(aWorm.score));
		try { Thread.sleep(delay); }	// L�t kombatanterna ta igen sig 1 sek.
