...
/**
 * An abstract base class for subclasses which convert character data in an
 * external encoding into Unicode characters.
 *
...
public abstract class ByteToCharConverter {
     ...
     * into an array of Unicode characters. This method allows a buffer by
     * buffer conversion of a data stream. The state of the conversion is saved
     * between calls to convert. Among other things, this means multibyte input
     * sequences can be split between calls. If a call to convert results in an
     * exception, the conversion may be continued by calling convert again with
     * suitably modified parameters. All conversions should be finished with a
     ...
     * be converted to Unicode. Thrown only when converter is not in
     * substitution mode.
    ...
    /**
     * Converts an array of bytes containing characters in an external encoding
     * into an array of Unicode characters. Unlike convert, this method does not
     * do incremental conversion. It assumes that the given input array contains
     ...
     * encountered by calling nextByteIndex.
     * <p>
     * This method uses substitution mode when performing the conversion. The
     * method setSubstitutionChars may be used to determine what characters are
     ...
     * @exception MalformedInputException if the input buffer contains any
     * sequence of chars that is illegal in the input character encoding. After
     * this exception is thrown, the method nextByteIndex can be called to
     * obtain the index of the first invalid input byte and getBadInputLength
     * can be called to determine the length of the invalid input.
     *
     * @see #nextByteIndex
     ...
     * a partial or invalid multibyte character sequence. flush will write what
     * it can to the output buffer and reset the converter before throwing this
     * exception. An additional call to flush is not required.
    ...
    /**
     * Returns the index of the character just past the last character written
     * by the previous call to convert.
     */
     ...
     * with the substitution character set by setSubstitionChars. When not in
