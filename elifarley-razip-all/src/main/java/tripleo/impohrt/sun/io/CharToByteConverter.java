 ...
 * questions.
 */
package tripleo.impohrt.sun.io;

import java.io.*;

/**
 * An abstract base class for subclasses which convert Unicode characters into
 * an external encoding.
 *
 * @author Asmus Freytag
 * @author Lloyd Honomichl, Novell, Inc.
 *
 * @deprecated Replaced by {@link java.nio.charset}. THIS API WILL BE REMOVED IN
 * J2SE 1.6.
 */
@Deprecated
public abstract class CharToByteConverter {
    ...
    /**
     * Converts an array of Unicode characters into an array of bytes in the
     * target character encoding. This method allows a buffer by buffer
     * conversion of a data stream. The state of the conversion is saved between
     * calls to convert. If a call to convert results in an exception, the
     ...
     * surrogates and \uFFFF or \uFFFE). After this exception is thrown, the
     * method nextCharIndex can be called to obtain the index of the first
     * invalid input character. The MalformedInputException can be queried for
     * the length of the invalid input.
     * @exception UnknownCharacterException for any character that that cannot
     * be converted to the external character encoding. Thrown only when
     * converter is not in substitution mode.
 ...
 /* For safety, use the public interface to charOff and byteOff, but
    ...
    /**
     * Converts an array of Unicode characters into an array of bytes in the
     * target character encoding. Unlike convert, this method does not do
     * incremental conversion. It assumes that the given input array contains
     * all the characters to be converted. The state of the converter is reset
     * at the beginning of this method and is left in the reset state on
     * successful termination. The converter is not reset if an exception is
     * thrown. This allows the caller to determine where the bad input was
     * encountered by calling nextCharIndex.
     * <p>
     * This method uses substitution mode when performing the conversion. The
     * method setSubstitutionBytes may be used to determine what bytes are
     * substituted. Even though substitution mode is used, the state of the
     * converter's substitution mode is not changed at the end of this method.
     *
     ...
     * surrogates and \uFFFF or \uFFFE). After this exception is thrown, the
     * method nextCharIndex can be called to obtain the index of the first
     * invalid input character and getBadInputLength can be called to determine
     * the length of the invalid input.
     *
     * @see #nextCharIndex
     ...
     * @exception ConversionBufferFullException if output array is filled before
     * all the output can be flushed. flush will write what it can to the output
     * buffer and remember its state. An additional call to flush with a new
     * output buffer will conclude the operation.
     */
    ...
    /**
     * Writes any remaining output to the output buffer and resets the converter
     * to its initial state. May only be called when substitution mode is turned
     * on, and never complains about malformed input (always substitutes).
     *
     * @param output byte array to receive flushed output.
     * @param outStart start writing to output array at this offset.
     * @param outEnd stop writing to output array at this offset (exclusive).
    ...
    /**
     * Returns true if the given character can be converted to the target
     * character encoding.
     *
    ...
    /**
     * Returns the maximum number of bytes needed to convert a char. Useful for
     * calculating the maximum output buffer size needed for a particular input
     * buffer.
     */
     ...
     * MalformedInputException thrown by the converter. If none have ever been
     * thrown, returns 0.
     */
     ...
     * successfully converted by the previous call to convert.
     */
    ...
    /**
     * Returns the index of the byte just past the last byte written by the
     * previous call to convert.
     */
     ...
     * converter will replace untranslatable characters in the source encoding
     * with the substitution character set by setSubstitutionBytes. When not in
     * substitution mode, the converter will throw an UnknownCharacterException
     * when it encounters untranslatable input.
     *
    ...
    /**
     * Sets the substitution bytes to use when the converter is in substitution
     * mode. The given bytes should represent a valid character in the target
     * character encoding and must not be longer than the value returned by
     * getMaxBytesPerChar for this converter.
     *
